    ...
    <div className="card p-4 space-y-2">
      ...
      <div className="flex flex-wrap gap-2">
        <button className="btn btn-primary" onClick={()=>{
          ...
          writeContract({ abi, address: addr, functionName:'createCircle', args:[name, desc] }, {
            onSuccess:()=>setMessage('Submitted createCircle'),
            onError:(e:any)=>setMessage(e.message)
          });
        }}>{isPending? 'Submitting…':'Create Circle'}</button>

        <button className="btn" onClick={()=>{
          const id = prompt('CircleId (bytes32 hex) to join?') as `0x${string}`;
          if (!id) return;
          writeContract({ abi, address: addr, functionName:'joinCircle', args:[id] }, {
